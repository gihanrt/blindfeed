...
public class ParagraphDAO {
	 ...
	 private JdbcTemplate jdbcTemplate;
	   ...
	   int data=this.jdbcTemplate.update(sql, location,userID,paragrpahID,bookID);
	    ...
	    public void updateTempory(int paragrpahID, int bookID){
	    	String sql="update `paragraph` set `Audio_Flag`=1, where `Paragraph_ID`= ? and `Book_Book_ID`= ?";
	    	int data=this.jdbcTemplate.update(sql,paragrpahID,bookID);
